...
def f(right, key):
    ...
    result = ''.join(sbox(temp[i*6:(i+1)*6], SBOXES[i]) for i in range(8))
...
def decrypt(cipher, key):
    ...
    for k in reversed(keys):
        left, right = right, xor(left, f(right, k))
...
cipher_bits = conn.recv(4096).decode()
print("Received cipher (bits):", cipher_bits)

key_input = input("Enter 64-bit key (same 8 ASCII chars used to encrypt): ")
key_bits = text_to_bits(key_input)

plain_bits = decrypt(cipher_bits, key_bits)
print("Decrypted:", bits_to_text(plain_bits))
