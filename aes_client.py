import socket

SBOX = [
    
]

RCON = [0x01,0x02,0x04,0x08,0x10,0x20,0x40,0x80,0x1b,0x36]

def xtime(a):
    return ((a << 1) ^ 0x1b) & 0xff if a & 0x80 else (a << 1) & 0xff

def gmul(a, b):
    p = 0
    for _ in range(8):
        if b & 1:
            p ^= a
        a = xtime(a)
        b >>= 1
    return p

def sub_bytes(state):
    return [[SBOX[state[r][c]] for c in range(4)] for r in range(4)]

def shift_rows(state):
    return [
        [state[0][0], state[0][1], state[0][2], state[0][3]],
        [state[1][1], state[1][2], state[1][3], state[1][0]],
        [state[2][2], state[2][3], state[2][0], state[2][1]],
        [state[3][3], state[3][0], state[3][1], state[3][2]]
    ]

def mix_columns(state):
    result = [[0]*4 for _ in range(4)]
    for c in range(4):
        col = [state[r][c] for r in range(4)]
        result[0][c] = gmul(col[0],2)^gmul(col[1],3)^col[2]^col[3]
        result[1][c] = col[0]^gmul(col[1],2)^gmul(col[2],3)^col[3]
        result[2][c] = col[0]^col[1]^gmul(col[2],2)^gmul(col[3],3)
        result[3][c] = gmul(col[0],3)^col[1]^col[2]^gmul(col[3],2)
    return result

def add_round_key(state, key_schedule, rnd):
    return [[state[r][c] ^ key_schedule[rnd*4+c][r] for c in range(4)] for r in range(4)]

def key_expansion(key):
    w = [[key[4*i+j] for j in range(4)] for i in range(4)]
    for i in range(4, 44):
        temp = w[i-1][:]
        if i % 4 == 0:
            temp = [SBOX[temp[1]]^RCON[i//4-1], SBOX[temp[2]], SBOX[temp[3]], SBOX[temp[0]]]
        w.append([w[i-4][j] ^ temp[j] for j in range(4)])
    return w

def bytes_to_state(data):
    return [[data[r+4*c] for c in range(4)] for r in range(4)]

def state_to_bytes(state):
    return bytes(state[r][c] for c in range(4) for r in range(4))

def encrypt_block(block, key):
    w = key_expansion(list(key))
    state = bytes_to_state(list(block))
    state = add_round_key(state, w, 0)
    for rnd in range(1, 11):
        state = sub_bytes(state)
        state = shift_rows(state)
        if rnd < 10:
            state = mix_columns(state)
        state = add_round_key(state, w, rnd)
    return state_to_bytes(state)

def pad(data):
    n = 16 - len(data) % 16
    return data + bytes([n] * n)

key_input = input("Enter 16-character AES key: ").encode()[:16].ljust(16, b'\x00')
plain_input = input("Enter plaintext: ").encode()
padded = pad(plain_input)

cipher = b''.join(encrypt_block(padded[i:i+16], key_input) for i in range(0, len(padded), 16))
cipher_hex = cipher.hex()
print("Encrypted (hex):", cipher_hex)

client = socket.socket()
client.connect(("localhost", 4444))
client.send(cipher_hex.encode())
print("Sent to server")
client.close()